/** Incremental SHA-256 for multi-gigabyte streams.
 *
 * WebCrypto's digest API requires one contiguous BufferSource. This small
 * implementation lets pack downloads be verified chunk-by-chunk without ever
 * allocating the complete SQLite file in JavaScript memory.
 */
export class Sha256 {
  ...
  private finish(): void {
    ...
    const bitHigh = Math.floor(bytes / 0x20000000);
    const bitLow = (bytes * 8) >>> 0;
    writeU32(this.buffer, 56, bitHigh);
    writeU32(this.buffer, 60, bitLow);
    this.compress(this.buffer);
  }
