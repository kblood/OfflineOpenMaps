...
export interface MapViewHandle {
  ...
  /**
   * Highlight one or more parcel/matrikel polygon rings on the map. Pass
   * `null` to clear. Each ring is a closed array of [lon, lat] pairs;
   * multiple rings render as a single MultiPolygon (each treated as an
   * outer ring — we don't model holes yet).
   */
  setParcel(rings: ReadonlyArray<ReadonlyArray<readonly [number, number]>> | null): void;
...
export const MapView = forwardRef<MapViewHandle, Props>(function MapView(
  ...
  // Same idea for the parcel highlight overlay.
  const lastParcelGeoJsonRef = useRef<GeoJSON.GeoJSON | null>(null);
  ...
  useEffect(() => {
    ...
    const ensureOverlayLayers = (): void => {
      ...
      // Parcel highlight: a translucent fill plus a stronger stroke so it
      // reads at any background colour. Layered ABOVE buildings so the
      // outline isn't hidden by the building fill at z14+, but BELOW the
      // route so a route's footprint stays visible through it.
      if (!map.getSource('current-parcel')) {
        map.addSource('current-parcel', {
          type: 'geojson',
          data: lastParcelGeoJsonRef.current ?? { type: 'FeatureCollection', features: [] },
        });
      }
      if (!map.getLayer('current-parcel-fill')) {
        map.addLayer(
          {
            id: 'current-parcel-fill',
            type: 'fill',
            source: 'current-parcel',
            paint: { 'fill-color': '#f0b000', 'fill-opacity': 0.18 },
          },
          'current-route-line',
        );
      }
      if (!map.getLayer('current-parcel-outline')) {
        map.addLayer(
          {
            id: 'current-parcel-outline',
            type: 'line',
            source: 'current-parcel',
            paint: { 'line-color': '#c08300', 'line-width': 2 },
          },
          'current-route-line',
        );
      }
    };
    map.on('load', ensureOverlayLayers);
    // `styledata` fires after every successful setStyle once the new
    // style's sources/layers are ready.
    map.on('styledata', ensureOverlayLayers);
  ...
  useImperativeHandle(ref, () => ({
    ...
    setParcel(rings) {
      const map = mapRef.current;
      const src = map?.getSource('current-parcel') as maplibregl.GeoJSONSource | undefined;
      let data: GeoJSON.GeoJSON;
      if (!rings || rings.length === 0) {
        data = { type: 'FeatureCollection', features: [] };
      } else {
        // Treat every ring as its own outer ring (no holes). Cast to the
        // mutable shape MapLibre expects.
        const polygons = rings.map(
          (ring) => [ring.map(([lon, lat]) => [lon, lat] as [number, number])] as Array<
            Array<[number, number]>
          >,
        );
        data = {
      ...
      lastParcelGeoJsonRef.current = data;
