...
interface Props {
  ...
  /**
   * Reverse-geocoded display name for each waypoint. Null while the lookup
   * is in flight or when nothing was within reverse-radius. The parent owns
   * the lookup so we don't refire it on every render of this panel.
   */
...
// Two-line waypoint button: resolved address on top, lat/lon underneath.
// While the reverse lookup is in flight (or returned nothing within radius),
// the subtitle line still shows the coordinates so the button is never empty
// once a point has been picked.
function WaypointButton({
  ...
  return (
    <button
      className={`waypoint ${picking ? 'primary' : ''}`}
      onClick={onClick}
